...
const password = ref('Alumno123!')
const localError = ref('')

watch(() => props.show, (newVal) => {
  if (newVal) {
    password.value = 'Alumno123!'
    localError.value = ''
  }
})
...
function handleSubmit() {
  ...
  if (!val) {
    localError.value = 'La contraseña inicial es obligatoria'
    return
  }
  if (val.length < 8) {
    ...
    <div class="modal-card">
      ...
      <form @submit.prevent="handleSubmit">
        <div class="modal-body">
          ...
          <div class="field">
            <label for="initial-student-pass">Contraseña Inicial * (Mínimo 8 caracteres)</label>
            <input
              id="initial-student-pass"
              v-model="password"
              type="text"
              placeholder="••••••••"
